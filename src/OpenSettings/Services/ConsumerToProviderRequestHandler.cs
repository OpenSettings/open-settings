...
{
    internal sealed class ConsumerToProviderRequestHandler : DelegatingHandler
    {
        ...
        public ConsumerToProviderRequestHandler(IOpenSettingsMemoryCache openSettingsMemoryCache, ITokenService tokenService, OpenSettingsConfiguration openSettingsConfiguration)
        {
        ...
        public async ValueTask<AuthenticationHeaderValue> GenerateTokenForMachineAsync(CancellationToken cancellationToken)
        {
            if (OpenSettingsDefaults.Caches.MachineTokenCacheEntryKey.TryGetValue<AuthenticationHeaderValue>(_openSettingsMemoryCache, out var authorization))
            {
