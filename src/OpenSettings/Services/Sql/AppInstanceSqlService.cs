...
{
    internal sealed class AppInstanceSqlService : IAppInstanceSqlService
    {
        private readonly OpenSettingsDbContext _context;
        private readonly IPasswordHasher<AppSqlModel> _passwordHasher;

        public AppInstanceSqlService(OpenSettingsDbContext context, IPasswordHasher<AppSqlModel> passwordHasher)
        {
            _context = context;
            _passwordHasher = passwordHasher;
        }

        public async Task<IResponse> CreateAppInstanceAsync(CreateInstanceInput input, CancellationToken cancellationToken)
        {
        ...
        {
            var instanceIdRule = ValidationRules.GreaterThanRule(nameof(input.AppInstanceId), input.AppInstanceId, 0);
