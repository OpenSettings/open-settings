...
{
    internal sealed class MachineToMachineTokenHandler : DelegatingHandler
    {
        private readonly IOpenSettingsMemoryCache _openSettingsMemoryCache;
        private readonly ITokenService _tokenService;
        private readonly OpenSettingsConfiguration _openSettingsConfiguration;

        public MachineToMachineTokenHandler(IOpenSettingsMemoryCache openSettingsMemoryCache, ITokenService tokenService, OpenSettingsConfiguration openSettingsConfiguration)
        {
        ...
        {
            if (!request.RequestUri?.LocalPath.EndsWith(OpenSettingsDefaults.Routes.V1.Token) ?? false)
            {
